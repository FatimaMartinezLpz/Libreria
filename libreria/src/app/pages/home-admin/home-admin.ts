...
export class HomeAdmin {
  ...
  obtenerUsuario(): void {
    ...
    if (usuarioData) {
      try {
        ...
        // Guardamos el rol en la propiedad para usar en el template
        this.rolUsuario = this.usuario.role || 'user';
  ...
  abrirGestionUsuarios(): void {
    ...
    this.cargarUsuarios();
    console.log('Se activó gestión de usuarios');
  ...
  guardarUsuario(): void {
    ...
    const datosAEnviar = { ...this.nuevoUsuario };

    // Si es edición y la contraseña está vacía, la eliminamos del payload
    if (this.usuarioEditando && !datosAEnviar.password) {
      delete datosAEnviar.password;
    }

    const operacion = this.usuarioEditando
      ? this.authService.updateUser(this.usuarioEditando.id, datosAEnviar)
      : this.authService.createUser(datosAEnviar);
